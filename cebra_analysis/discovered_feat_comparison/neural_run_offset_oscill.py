...
for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(X_unique, groups=unique_groups)):
        ...
        X_train = pd.DataFrame(X_train.loc[:, "ECOG_LEFT_fft_high beta"])#, "ECOG_RIGHT_fft_high gamma"]]
        X_test = pd.DataFrame(X_test.loc[:, "ECOG_LEFT_fft_high beta"])#, "ECOG_RIGHT_fft_high gamma"]]
    
        multi_cebra_model.fit(X_train, y_train)
        tmp_file = Path("/home/garullie/CEBRA_analysis/CEBRA_train/0_higher_offset/discovered_feat_comparison/models", f'run_splits_{fold_idx}_beta.pt')
        ...
        with open(f"./neural_predictions/predictions_neu_{fold_idx}_beta.pkl", 'wb') as handle:
